...
#define RTC__IOPort__Data		RTC__BaseIOPort + 0x01
...
void RTC_Startup()
{
    ...
    outb(RTC__IOPort__Address, 0x0B);
    uint8_t modes = inb(RTC__IOPort__Data);
...
{
    do { outb(RTC__IOPort__Address, 0x0A); }
    while ((inb(RTC__IOPort__Data) & 0x80) != 0);
}
...
{
    RTC_WaitForReady();
    outb(RTC__IOPort__Address, 0x00);
    if (gRTC_BinaryCodedDecimalMode) return RTC_FromBCD(inb(RTC__IOPort__Data));
    return inb(RTC__IOPort__Data);
}

uint8_t RTC_GetMinute()
{
    RTC_WaitForReady();
    outb(RTC__IOPort__Address, 0x02);
    if (gRTC_BinaryCodedDecimalMode) return RTC_FromBCD(inb(RTC__IOPort__Data));
    return inb(RTC__IOPort__Data);
}
...
{
    RTC_WaitForReady();
    outb(RTC__IOPort__Address, 0x04);
    if (gRTC_BinaryCodedDecimalMode)
    {
        if (gRTC_MilitaryMode) return RTC_FromBCD(inb(RTC__IOPort__Data));
        uint8_t b = inb(RTC__IOPort__Data);
    ...
    {
        if (gRTC_MilitaryMode) return inb(RTC__IOPort__Data);
        uint8_t b = inb(RTC__IOPort__Data);
...
{
    RTC_WaitForReady();
    outb(RTC__IOPort__Address, 0x06);
    if (gRTC_BinaryCodedDecimalMode) return RTC_FromBCD(inb(RTC__IOPort__Data));
    return inb(RTC__IOPort__Data);
}
...
{
    RTC_WaitForReady();
    outb(RTC__IOPort__Address, 0x07);
    if (gRTC_BinaryCodedDecimalMode) return RTC_FromBCD(inb(RTC__IOPort__Data));
    return inb(RTC__IOPort__Data);
}
...
{
    RTC_WaitForReady();
    outb(RTC__IOPort__Address, 0x08);
    if (gRTC_BinaryCodedDecimalMode) return RTC_FromBCD(inb(RTC__IOPort__Data));
    return inb(RTC__IOPort__Data);
}
...
{
    RTC_WaitForReady();
    outb(RTC__IOPort__Address, 0x09);
    if (gRTC_BinaryCodedDecimalMode) return RTC_FromBCD(inb(RTC__IOPort__Data));
    return inb(RTC__IOPort__Data);
}
...
{
    RTC_WaitForReady();
    outb(RTC__IOPort__Address, 0x32);
    if (gRTC_BinaryCodedDecimalMode) return RTC_FromBCD(inb(RTC__IOPort__Data));
    return inb(RTC__IOPort__Data);
}
