...
//					newLocalVariable->SSAData->LifeStarted = pMethod->IRCodes[codeNode->Instructions[instructionIndex]];
				...
				{
					newLocalVariable->SSAData->Derived->SSAData->DerivedIterations++;
					newLocalVariable->SSAData->Iteration = newLocalVariable->SSAData->Derived->SSAData->DerivedIterations;
					instruction->Destination.Data.LocalVariable.LocalVariableIndex = newLocalVariable->LocalVariableIndex;
...
void IROptimizer_LeaveSSA(IRMethod* pMethod, IRCodeNode** pNodes, uint32_t pNodesCount, uint32_t pOriginalLocalCount)
{
	IRInstruction* instruction = NULL;
	...
	if (pMethod->LocalVariableCount)
	{
		...
		//		bool_t unused = TRUE;
		//		uint32_t firstInstructionForPhi = phi->Result->SSAData->LifeStarted->IRLocation;
		//		//for (uint32_t sourceIndex = 0; sourceIndex < phi->ArgumentsCount; ++sourceIndex)
		//		//{
		//		//	if (phi->Arguments[sourceIndex]->SSAData->LifeStarted->IRLocation < firstInstructionForPhi)
		//		//		firstInstructionForPhi = phi->Arguments[sourceIndex]->SSAData->LifeStarted->IRLocation;
		//		//}
		//		printf("PHI: Result @ %u, this node starts @ 0x%X, but first use @ 0x%X\n", (unsigned int)phi->Result->LocalVariableIndex, (unsigned int)pMethod->IRCodes[codeNode->Instructions[0]]->ILLocation, (unsigned int)pMethod->IRCodes[firstInstructionForPhi]->ILLocation);
		//		for (uint32_t instructionIndex = pMethod->IRCodesCount - 1; instructionIndex >= firstInstructionForPhi; --instructionIndex)
		//		{
		//			instruction = pMethod->IRCodes[instructionIndex];
		...
		IRLocalVariable* oldLocalVariable = NULL;
		IRLocalVariable* newLocalVariable = NULL;
		uint32_t derivedCount = pOriginalLocalCount;
		...
		for (uint32_t nodeIndex = 0; nodeIndex < pNodesCount; ++nodeIndex)
		{
			...
			if (codeNode->SourceFrontiersCount >= 2)
			{
				...
				for (uint32_t derivedIndex = 0; derivedIndex < derivedCount; ++derivedIndex)
				{
					...
					{
						bool_t allSame = TRUE;
						IRLocalVariable* firstFrontierLocal = IROptimizer_GetFinalIteration(pMethod, codeNode->SourceFrontiers[0], derivedIndex);
						for (uint32_t frontierIndex = 0; frontierIndex < codeNode->SourceFrontiersCount; ++frontierIndex)
						{
							if (IROptimizer_GetFinalIteration(pMethod, codeNode->SourceFrontiers[frontierIndex], derivedIndex)->LocalVariableIndex != firstFrontierLocal->LocalVariableIndex)
							{
								allSame = FALSE;
								break;
							}
						...
						Log_WriteLine(LOGLEVEL__Optimize_SSA, "Phi Local Assignment at instruction 0x%X (Node %u) to %u, iteration %u (Derived %u)", (unsigned int)pMethod->IRCodes[codeNode->Instructions[0]]->ILLocation, (unsigned int)codeNode->Index, (unsigned int)newLocalVariable->LocalVariableIndex, (unsigned int)newLocalVariable->SSAData->Iteration, (unsigned int)newLocalVariable->SSAData->Derived->LocalVariableIndex);
						...
						for (uint32_t frontierIndex = 0; frontierIndex < codeNode->SourceFrontiersCount; ++frontierIndex)
						{
							...
							if (oldLocalVariable->SSAData->LifeStarted->ILLocation < newLocalVariable->SSAData->LifeStarted->ILLocation)
								newLocalVariable->SSAData->LifeStarted = oldLocalVariable->SSAData->LifeStarted;
							codeNode->SourceFrontiers[frontierIndex]->FinalIterations[derivedIndex] = newLocalVariable;
							IRPhi_AddArgument(phi, oldLocalVariable);
							Log_WriteLine(LOGLEVEL__Optimize_SSA, "    From %u, iteration %u, starting @ 0x%X", (unsigned int)oldLocalVariable->LocalVariableIndex, (unsigned int)oldLocalVariable->SSAData->Iteration, (unsigned int)oldLocalVariable->SSAData->LifeStarted->ILLocation);
							memset(retargettedNodes, 0x00, pNodesCount * sizeof(bool_t));
							IROptimizer_RetargetLocalSources(pMethod, pNodes[0], 0, oldLocalVariable, newLocalVariable, retargettedNodes);
						}
					}
				}
			}
				...
				if (lifeEndsHere)
				{
					...
					for (uint32_t nodeIndex = 0; nodeIndex < pNodesCount; ++nodeIndex)
					{
							...
							codeNode->Instructions[codeNode->InstructionsCount - 1] >= instruction->IRLocation)
						{
							...
							if (lastNodeInstruction->Opcode == IROpcode_Branch && lastNodeInstruction->ILLocation >= ((IRInstruction*)lastNodeInstruction->Arg2)->ILLocation)
							{
