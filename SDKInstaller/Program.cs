...
namespace SDKInstaller
{
    ...
    {
        private static string sURL = "https://dl.dropboxusercontent.com/u/35394461/ProtonCPP/";
        private static string sPackageList = "SDKPackages.xml";
        private static string sHost = "win32";
        private static string sTarget = "i686";
        ...
        private static void Main(string[] pCommandLine)
        {
            ...
            if (!DownloadFile(string.Format("SDK-{0}-{1}.7z", sHost, sTarget))) return;
            Extract(string.Format("SDK-{0}-{1}.7z", sHost, sTarget));

            File.Delete(Path.GetFullPath(sExtractorFile));
